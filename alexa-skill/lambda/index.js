 ...
 * Alexa <-> Claude bridge.
 *
 * Zero npm dependencies on purpose: this file can be pasted directly into
 * the AWS Lambda inline code editor (Node.js 18.x or 20.x runtime, both of
 * which ship a native `fetch`). No build step, no zip upload required.
 *
 ...
 *                           does not match are rejected (recommended once you
 *                           know your skill ID, see README).
 */
...
async function handleIntent(event) {
  ...
  if (intentName === 'AskClaudeIntent') {
    ...
    if (!ANTHROPIC_API_KEY) {
      ...
      return buildResponse(
        'Falta configurar la clave de la API de Claude en el servidor.',
        true
      );
    }
